...
public class App {
	...
	public static void main(String[] args) {
        ...
        //aeropuerto Con Nombre Nombre2 Que Tenga un Vuelo Con Mas De Tres Billetes Y Un Pasajero De 18 Años
        ejercicioStreams1(vuelos);
        
        //Obtener todos los pasajeros a partir de la lista de vuelos
        ejercicioStreams2(vuelos);

	}

	private static void ejercicioStreams2(List<Vuelo> vuelos) {
		vuelos.stream().map(Vuelo::getBilletes).flatMap(billete->billete.stream()).forEach(b->System.out.println(b.getPasajero()));
	}

	private static void ejercicioStreams1(List<Vuelo> vuelos) {
		System.out.println(vuelos.stream().filter(a->a.getBilletes().size()>3 && a.getBilletes().stream().anyMatch(bill->bill.getPasajero().getAge().equals(18))).map(Vuelo::getDestino).filter(aer->aer.getName().equals("nombre2")).findFirst().orElse(new Aeropuerto()));
